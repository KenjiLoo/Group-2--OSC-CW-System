...
void RoundRobinWithoutOverhead(int Time_Quantum, int Burst_Time[], int NumberOfProcess) {
  ...
  int Waiting_Time = 0;
  int WT = 0;
  bool isNotfirst_execution[MAX] = {false};
  int diff = 0;
  ...
  for(i = 0; x != 0; i++) {
    ...
    if(temp[i] == 0 && counter == 1) {
      Waiting_Time = (i * Time_Quantum) - diff;
      ...
      printf("\n\tP%lu %15d %18d %17d", i + 1, Burst_Time[i], total, Waiting_Time);
